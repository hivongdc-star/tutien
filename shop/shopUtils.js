...
function getItem(itemId){ const catalog = loadItems(); return catalog[itemId] || null; }
...
function buyItem(buyerId,itemId,qty=1){
  ...
  if (it.type === "mining_tool") {
    ...
    for (let k = 0; k < q; k++) {
      ...
      buyer.mining.tools.push({
        ...
        bonusRare: Number(it.bonusRare ?? it.bonus ?? 0),
        boughtAt: Date.now(),
      });
  ...
  const tierLine = it.tier ? ` • ${tierText(it.tier)}` : "";
  const name = `${it.emoji||""} ${it.name}`.trim();
  const qtyTxt = q > 1 ? `x${q} ` : "";
  return { ok:true, message:`✅ Đã đưa vào hành trang: ${qtyTxt}**${name}**${tierLine} • Tổng giá **${fmtLT(total)} LT**.` };
}

module.exports={ listItems, getItem, buyItem };
